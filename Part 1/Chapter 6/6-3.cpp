...
/*  COMMENT
 *  Visual Studio 2019 19.23 (partial) supports concepts that you can find in the Standard Libaray headers.
...
T mode(const T *values, std::size_t length) {
	...
	if (!values)     throw std::invalid_argument{ "mode(const T *values, std::size_t length): null pointer" };
...
int main() {
	...
	float float_array[]{2.0f, 8.0f, 1.0f, 1.0f, 2.0f, 9.0f, 1.0f, 5.0f, 5.0f, 3.0f};
